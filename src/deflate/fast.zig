...
const Loop = struct {
    ...
    fn has_margin(self: *const Loop) bool {
        return self.input.len - self.position >= input_slack and self.output.len - self.written >= output_slack;
    }

    /// Fills the buffer to at least `refill_bits` bits with one 8-octet load. The bits above
    /// `count` repeat the input's next octets, which a later load writes again unchanged.
    fn refill(self: *Loop) void {
        ...
        const taken = (@bitSizeOf(u64) - 1 - self.count) / @bitSizeOf(u8);
        self.position += taken;
        self.count += taken * @bitSizeOf(u8);
    }

    fn consume(self: *Loop, count: u32) void {
...
pub fn run(codes: Codes, history: History, bits: *codec.BitReader, writer: *codec.Writer) End {
    var loop: Loop = .{
...
/// Decodes one literal/length symbol, and the distance a length takes. Returns why the loop
/// stops, or null to go on.
fn step(loop: *Loop, codes: Codes, history: History) ?End {
    var entry = codes.literal_length_table.lookup(loop.buffer);
    if (entry.kind == .long) entry = resolve_literal_length(codes, loop.buffer) orelse return .checked;
    switch (entry.kind) {
        .literal => {
            loop.output[loop.written] = @intCast(entry.value);
            loop.written += 1;
...
fn copy_pair(loop: *Loop, codes: Codes, history: History, length: lookup.Entry) ?End {
    ...
    var distance_entry = codes.distance_table.lookup(loop.buffer >> @intCast(used));
...
/// Copies `len` octets to `target` from `distance` before it, in chunks where the distance leaves
/// room for one, a fill for a distance of 1, and octet by octet otherwise. A chunk may write up to
/// `copy_chunk_len - 1` octets past `len`, into the margin, and reads only octets written before.
