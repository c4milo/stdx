//! The zlib decoder's split property, over inputs the fuzzer or a seed draws: any input, valid or
...
fn check_split(input: []const u8, seed: u64) !void {
    ...
    zlib.init(&decoder, .{});
    const whole: Verdict = if (zlib.decode(&decoder, input, &whole_output)) |progress| .{ .progress = progress } else |err| .{ .refused = err };
...
/// A valid stream to corrupt: a stored block, then a fixed block of literals and a match.
fn valid_stream(stream: *Stream) void {
    const stored = "the stored block of a zlib stream";
    const literals = "fixed";
    decoder_test.header(stream, decoder_test.deflate_32k, 0);
...
test "fuzz the decoder's split property" {
    var stream: Stream = .{};
    valid_stream(&stream);
    try testing.fuzz({}, fuzz_one, .{ .corpus = &.{ "", "\x78\x9c", stream.slice() } });
}
