...
Nothing here is measured yet. Design §8 step 2 fills the table on the Linux machine decision 10
requires, with `bench/costs/`, and writes the machine down below. macOS publishes no number
(decision 10). Each cell is the median of five runs, with the spread, in nanoseconds and in cycles
from `perf stat`.
...
|---|---|
| CPU model | |
| Cores used, and how they are pinned | |
| Frequency governor | |
| Kernel | |
...
| Cost | What stdx does that pays it | Median, ns | Median, cycles | Spread | Claims priced against it |
|---|---|---|---|---|---|
...
Each row is a microbenchmark in `bench/costs/`, built ReleaseSafe, run on one pinned core with the
warm-up discarded. It lands with design §8 step 2.
